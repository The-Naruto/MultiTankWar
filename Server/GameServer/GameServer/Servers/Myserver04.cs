...
{
    internal class MyServer04 : IHostedService
    {
        ILogger<MyServer04> _logger;
        Socket _listenfd;
        //客户端Socket及状态信息
        readonly Dictionary<Socket, ClientState> _clients = [];
        public MyServer04(ILogger<MyServer04> logger)
        {
            _logger = logger;
        }
        ...
        private bool ReadClientfd(Socket clientfd)
        {
            try
            {
              ...
              // byte[] sendBytes = System.Text.Encoding.UTF8.GetBytes(sendStr);
                ...
                {
                    cs.socket.Send(state.readBuff, count,0);
                }
                state.readBuff = new byte[1024];
            }
            catch (Exception e)
            {
                _clients.Remove(clientfd);
